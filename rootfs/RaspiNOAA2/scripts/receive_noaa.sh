...
if (( FREE_MEMORY == 0 )); then
    ...
    in_mem=false
elif [ "$FREE_MEMORY" -lt $NOAA_MEMORY_THRESHOLD ]; then
...
if [ "${PRODUCE_NOAA_PRISTINE_HISTOGRAM}" == "true" ]; then
  ...
  rm -f "${IMAGE_FILE_BASE}-histogram-a.jpg" "${IMAGE_FILE_BASE}-histogram-b.jpg" "${IMAGE_THUMB_BASE}-histogram-a.jpg" "${IMAGE_THUMB_BASE}-histogram-b.jpg" "${tmp_dir}/${FILENAME_BASE}-a.png" "${tmp_dir}/${FILENAME_BASE}-b.png"

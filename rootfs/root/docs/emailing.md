...
`/root/.msmtprc` file that enables configuring a gmail account to email images with.
...
**WARNING**: MAKE SURE YOU KEEP THE CONFIG FILE LOCKED DOWN! This file (`/root/.msmtprc`) will contain your email credentials,
...
align your environment to install dependencies and requirements, including a `/root/.msmtprc` file that has defaults for
configuring a gmail account to send emails.

## Configure Email Settings

Once the installer has been run, you should see a settings file in your user home directory named `/root/.msmtprc`.
...
and use this within the `/root/.msmtprc` file for the `password` setting.
